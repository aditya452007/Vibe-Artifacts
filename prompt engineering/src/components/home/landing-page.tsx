      ...
      <section className="relative min-h-[90vh] flex flex-col items-center justify-center text-center px-6 pt-32 overflow-hidden bg-gradient-to-b from-background via-background to-surface/50">
        ...
        >
          ...
          <div className="pt-8 flex flex-col sm:flex-row gap-4 items-center">
            <Link href={session ? "/docs" : "/signup"} passHref>
                <motion.div
                ...
                >
                <span>{session ? "Explore Documentation" : "Start Engineering"}</span>
            ...
            {!session && (
                <Link href="/login" passHref>
                    ...
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="group relative inline-flex items-center gap-2 px-8 py-4 bg-surface/50 border border-border/50 text-foreground rounded-full font-medium text-lg tracking-tight backdrop-blur-md hover:bg-surface/80 transition-all"
                    >
                    <span>Sign In</span>
                    </motion.div>
                </Link>
